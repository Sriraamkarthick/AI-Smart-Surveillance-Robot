...
IP_CAMERA_URL = 0  #http://172.17.20.117/mjpeg/1
DETECTED_IMAGES_DIR = 'detected_images'
EXCEL_FILE = 'Detections.xlsx'  # Updated Excel file name
...
if not os.path.isfile(EXCEL_FILE):
    df = pd.DataFrame(columns=['Timestamp', 'Class', 'Confidence', 'Coordinates'])  # Define columns
    try:
...
def gen_frames():
    """Generate frames from the camera stream and process detections."""
    global cap
    while True:
        if not cap.isOpened():
            cap = initialize_camera(IP_CAMERA_URL)
        ...
        # Initialize an empty list to store detection data
        detection_data = []
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if confidence > 0.5:  # Only consider predictions with confidence > 0.5
                    ...
                    detection_data.append({'Timestamp': timestamp, 'Class': class_name, 'Confidence': confidence, 'Coordinates': coordinates})

        # Log detection data to Excel file
        if detection_data:
            try:
...
def capture_image():
    ...
    if success:
        ...
        if os.path.isfile(EXCEL_FILE):
            ...
            df = pd.DataFrame(columns=['Timestamp', 'Class', 'Confidence', 'Coordinates'])

        # Append new data (Note: Image capture logging won't include 'Class' or 'Confidence')
        new_entry = pd.DataFrame([[timestamp, 'Image Capture', '-', '-']], columns=['Timestamp', 'Class', 'Confidence', 'Coordinates'])
